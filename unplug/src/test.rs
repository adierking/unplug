...
pub const TEST_WAV: &[u8] = include_bytes!("test/ionpack.wav");
/// Offset of the data section in `TEST_WAV`
pub const TEST_WAV_DATA_OFFSET: usize = 0x24;
/// Test sound with some cue points
pub const TEST_WAV_CUES: &[u8] = include_bytes!("test/ionpack-cues.wav");
/// Test sound with a gain of -30dB
pub const TEST_WAV_VOL: &[u8] = include_bytes!("test/ionpack-vol.wav");

/// Test sound left channel encoded to GC ADPCM format
pub const TEST_WAV_LEFT_DSP: &[u8] = include_bytes!("test/ionpack-dsp-left.bin");
/// End address of the encoded test sound
pub const TEST_WAV_DSP_END_ADDRESS: usize = 0x30af8;
/// Test sound left channel coefficients
pub const TEST_WAV_LEFT_COEFFICIENTS: [i16; 16] =
...
pub const TEST_WAV_RIGHT_DSP: &[u8] = include_bytes!("test/ionpack-dsp-right.bin");
/// Test sound right channel coefficients
pub const TEST_WAV_RIGHT_COEFFICIENTS: [i16; 16] =
    [49, 1829, 3542, -1781, 784, 1112, 3720, -1707, 374, 1605, 3677, -1738, 1630, 371, 3840, -1800];

/// `ionpack.wav` as PCMS32LE
pub const TEST_WAV_S32: &[u8] = include_bytes!("test/ionpack-s32.wav");
pub const TEST_WAV_S32_DATA_OFFSET: usize = 0x66;

/// `ionpack.wav` as PCMF32LE
pub const TEST_WAV_F32: &[u8] = include_bytes!("test/ionpack-f32.wav");
pub const TEST_WAV_F32_DATA_OFFSET: usize = 0x72;

/// `ionpack.wav` as FLAC
pub const TEST_FLAC: &[u8] = include_bytes!("test/ionpack.flac");

/// `ionpack.wav` as MP3
pub const TEST_MP3: &[u8] = include_bytes!("test/ionpack.mp3");
/// `ionpack.wav` encoded to MP3 and back to WAV
pub const TEST_MP3_WAV: &[u8] = include_bytes!("test/ionpack-mp3.wav");

/// `ionpack.wav` as Ogg Vorbis
pub const TEST_OGG: &[u8] = include_bytes!("test/ionpack.ogg");
/// `ionpack.wav` encoded to Ogg Vorbis and back to WAV
pub const TEST_OGG_WAV: &[u8] = include_bytes!("test/ionpack-ogg.wav");

/// Dummy banner file
pub const TEST_BANNER: &[u8] = include_bytes!("test/opening.bnr");
...
pub fn write_and_read<T>(val: &T) -> T
where
...
/// Writes a value to a byte array and reads it back.
/// Use `assert_write_and_read!()` instead of calling this directly.
pub fn write_and_read_custom<T, F>(val: &T, read: F) -> T
where
...
pub fn assert_samples_close<F: FormatTag>(
    actual: &Samples<'_, F>,
